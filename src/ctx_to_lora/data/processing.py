...
# Number of workers for dataset .map() / .filter() calls.
# Override with D2L_NUM_PROC env var (useful in containers where
# /dev/shm is too small for 16 multiprocessing workers).
_DEFAULT_NUM_PROC = int(os.environ.get("D2L_NUM_PROC", "16"))
...
def load_answers(ds_name, split):
    ...
    ds = ds.map(extract_ans, num_proc=_DEFAULT_NUM_PROC, remove_columns=ds.column_names)
    return ds
...
def get_tokenized_dataset(
    ...
    if ("train" in split) and is_caching_enabled():
        ...
        tokenized_ds.save_to_disk(ds_path, num_proc=_DEFAULT_NUM_PROC)
...
def construct_and_tokenize_ctx_qa(
    ...
    if "input_ids" in ds.column_names and "response_start_end" in ds.column_names:
        ...
        tokenized_ds = ds.map(get_labels_from_input_ids, num_proc=_DEFAULT_NUM_PROC)
    else:
